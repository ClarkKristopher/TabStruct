...
# =                                                              =
# ================================================================
class BaseSynthcityGenerator(BaseGenerator):
    ...
    def _fit(self, data_module):
        # Some Synthcity models has intrinsic data preprocessing, so we should reverse the one-hot encoding for categorical features
        ...
        self._fit_synthcity(train_data_dict)
    ...
    def format_train_data(self, data_module):
        ...
        # Synthcity does not handle string categories, and thus we need to apply ordinal encoding
    ...
    def _generate(self, class2synthetic_samples):
        synthetic_data_dict = self._generate_synthcity(class2synthetic_samples)

        # Synthcity has intrinsic data preprocessing, so we should one-hot encode the synthetic data for uniform evaluation
        if self.disable_feature_encoding:
    ...
    @abstractmethod
    def _fit_synthcity(self, data_module):
        raise NotImplementedError("This method has to be implemented by the sub class")

    @abstractmethod
    def _generate_synthcity(self, class2synthetic_samples):
        raise NotImplementedError("This method has to be implemented by the sub class")


class BaseSynthcityConditionalGenerator(BaseSynthcityGenerator):

    def __init__(self, args):
        super().__init__(args)

    def _fit_synthcity(self, train_data_dict):
...
class BaseSynthcityJointGenerator(BaseSynthcityGenerator):
...
class BaseMixedGenerator(BaseGenerator):
    ...
    def prepare_data(self, data_module):
        ...
        # For classification, we save a sample per class to duplicate when TabSyn cannot generate for some classes..
        if self.args.task == "classification":
            ...
            if self.args.task == "classification":
                if pd.unique(y_syn_df.iloc[:, -1]).size != len(class2synthetic_samples.keys()):
                    ...
                    TerminalIO.print(
                        ...
                        f"TabSyn cannot synthesise samples for some classes. Thus we will use example samples instead.",
                        color=TerminalIO.WARNING,
                    )
                    X_syn_df = pd.concat(
