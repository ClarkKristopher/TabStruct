...
from .model import Decoder_model, Encoder_model, Model_VAE
...
def run_with_processed_data(
    ...
    ckpt_dir = os.path.join(log_dir, "ckpt", "vae")
    ...
    categories = [len(np.unique(X_train_cat[:, i])) for i in range(X_train_cat.shape[1])]
