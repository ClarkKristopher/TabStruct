...
class TabDiffDataset(Dataset):
    def __init__(
        self, dataname, data_dir, info, isTrain=True, y_only=False, dequant_dist="none", int_dequant_factor=0.0
    ):
...
class CustomDataset(Dataset):
    def __init__(
        ...
        self.X = torch.cat((X_num, X_cat), dim=1)

        self.d_numerical = X_num.shape[1]
        self.categories = np.array([len(np.unique(X_cat[:, i])) for i in range(X_cat.shape[1])])
