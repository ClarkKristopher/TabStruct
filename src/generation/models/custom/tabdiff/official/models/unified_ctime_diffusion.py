...
class UnifiedCtimeDiffusion(torch.nn.Module):
    ...
    def sample(self, num_samples):
        ...
        if self.sampler_params["stochastic_sampler"]:
            gamma = (
                min(S_churn / self.num_timesteps, np.sqrt(2) - 1) * (S_min <= sigma_num_cur) * (sigma_num_cur <= S_max)
            )
    ...
    def pad(self, x, pad_value):
        ...
        padded_ = [
            torch.cat(
                (t, pad_value * torch.ones(*(t.shape[:-1]), max_K - t.shape[-1], dtype=t.dtype, device=t.device)),
                dim=-1,
            )
            for t in splited
        ]
    ...
    def _mdlm_update(self, log_p_x0, x, alpha_t, alpha_s):
        ...
        dummy_mask = torch.tensor(
            [[(1 if i <= mask_idx else 0) for i in range(max(self.mask_index + 1))] for mask_idx in self.mask_index],
            device=q_xs.device,
        )
    ...
    def sample_impute(self, x_num, x_cat, num_mask_idx, cat_mask_idx, resample_rounds, impute_condition, w_num, w_cat):
        ...
        if impute_condition == "x_t":
            z_norm = (
                x_num + torch.randn((b, self.num_numerical_features), device=device) * sigma_num_cur[-1]
        ...
        for i in pbar:
            for u in range(resample_rounds):
                ...
                if impute_condition == "x_t":
                    z_norm_known = (
                        x_num + torch.randn((b, self.num_numerical_features), device=device) * sigma_num_next[i]
                    )
                    move_chance = (
