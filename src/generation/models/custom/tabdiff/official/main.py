...
from .utils_train import CustomDataset, TabDiffDataset
...
def main(args):
    ...
    if args.mode == "train":
        ...
        if ckpt_path is None:
            ...
            assert (
                ckpt_path_arr
            ), f"Cannot not infer ckpt_path from {ckpt_parent_path}, please make sure that you first train a model before testing!"
    ...
    if args.mode == "train":
        ...
        result_save_path = model_save_path.replace(
            "ckpt", "result"
        )  # i.e., f'{curr_dir}/results/{dataname}/{exp_name}'
    elif args.mode == "test":
        if args.report:
            result_save_path = f"eval/report_runs/{exp_name}/{dataname}"
        else:
            result_save_path = os.path.dirname(ckpt_path).replace(
                "ckpt", "result"
            )  # infer the exp_name from the ckpt_name
    ...
    if args.y_only:
        ...
        if (
            main_model_configs["diffusion_params"]["scheduler"] == "power_mean_per_column"
            ...
            if info["task_type"] == "regression":
                ...
                raw_config["diffusion_params"]["noise_schedule_params"]["rho"] = noise_schedule.rho()[
                    0
                ...
                raw_config["diffusion_params"]["noise_schedule_params"]["k"] = noise_schedule.k()[
                    0
                ].item()  # the target col is placed at the first position
...
def run_with_processed_data(
    ...
    train_data = CustomDataset(X_train_num, X_train_cat)
    train_loader = DataLoader(
        train_data,
        batch_size=batch_size,
        shuffle=True,
    ...
    # parse the data info for training
    ...
    if not raw_config["diffusion_params"]["non_learnable_schedule"]:
        raw_config["diffusion_params"]["scheduler"] = "power_mean_per_column"
        raw_config["diffusion_params"]["cat_scheduler"] = "log_linear_per_column"
    diffusion = UnifiedCtimeDiffusion(
        num_classes=categories,
    ...
    # === Train the model ===
    # Creat model_save and result paths
    model_save_path = os.path.join(log_dir, "ckpt")
