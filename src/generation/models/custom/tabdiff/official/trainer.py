...
class Trainer:
    def __init__(
        ...
        **kwargs,
        ...
        self.scheduler = ReduceLROnPlateau(
            self.optimizer, mode="min", factor=factor, patience=reduce_lr_patience, verbose=True
        )
    ...
    def run_loop(self):
        ...
        for epoch in range(start_epoch, self.steps):
            ...
            if total_loss < best_loss:
                ...
                torch.save(
                    state_dicts, os.path.join(self.model_save_path, f"best_model_{np.round(total_loss,4)}_{epoch+1}.pt")
                )
                patience = 0
            else:
            ...
            if (epoch + 1) % self.check_val_every == 0 and self.test_dataset is not None:
                state_dicts = {
                    "denoise_fn": self.diffusion._denoise_fn.state_dict(),
                    "num_schedule": self.diffusion.num_schedule.state_dict(),
                    "cat_schedule": self.diffusion.cat_schedule.state_dict(),
    ...
    def evaluate_generation(self, save_metric_details=False, plot_density=False, ema=False):
        ...
        num_samples = (
            self.num_samples_to_generate if self.num_samples_to_generate else self.metrics.real_data_size
    ...
    def sample_without_recover_data(self, num_samples, keep_nan_samples=True):
        ...
        syn_data = syn_data.float().cpu().numpy()
        syn_num = syn_data[:, : self.dataset.d_numerical]
        syn_cat = syn_data[:, self.dataset.d_numerical :]
    ...
    def test_impute(
        ...
        with torch.no_grad():
            ...
            for trial in range(trail_start, trail_start + trial_size):
                ...
                if torch.any(
                    (syn_data[:, d_numerical + 1 :]).max(dim=0).values > (x_cat_train[:, 1:]).max(dim=0).values
    ...
    def update_training_config(self, new_config):
        ...
        self.weight_decay = new_config["weight_decay"]
        self.steps = new_config["steps"]
        self.batch_size = new_config["batch_size"]
        self.check_val_every = new_config["check_val_every"]
