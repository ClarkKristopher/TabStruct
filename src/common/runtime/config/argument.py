...
from ... import (WANDB_ENTITY, WANDB_PROJECT, generator_list, model_to_do_list, predictior_list, unstable_dataset_list,
...
def add_lit_train_setup(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--enable_lookahead_optimizer",
        action="store_true",
        dest="enable_lookahead_optimizer",
        help="Enhance the vanilla optimiser with Lookahead.",
    )
    parser.set_defaults(enable_lookahead_optimizer=False)
    # Scheduler
    parser.add_argument(
        "--lr_scheduler",
        type=str,
...
def add_eval_setup(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    parser.set_defaults(eval_stats=True)
    parser.add_argument(
    ...
    parser.add_argument(
        "--enable_eval_causality",
        action="store_true",
        dest="eval_causality",
        help="Evaluate the causality metrics of a generative model.",
    )
    parser.set_defaults(eval_causality=False)
...
def cross_update(args):
    ...
    if args.synthetic_data_path is None:
        ...
        if args.model in ["great"]:
            args.categorical_transform = "ordinal"
        elif args.model in ["tabsyn", "tabdiff"]:
            args.categorical_transform = "ordinal"
            if args.dataset in unstable_dataset_list:
                args.categorical_as_numerical = True

    # === For some generators that only support numerical features, we can transform all features to numerical ===
    if args.model in ["nrgboost"]:
        args.categorical_as_numerical = True
