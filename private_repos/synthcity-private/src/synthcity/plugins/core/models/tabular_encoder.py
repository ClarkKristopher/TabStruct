...
class FeatureInfo(BaseModel):
    ...
    def _transform_validator(cls: Any, v: Any) -> Any:
        if not (hasattr(v, "fit") and hasattr(v, "transform") and hasattr(v, "inverse_transform")):
...
class TabularEncoder(TransformerMixin, BaseEstimator):
    ...
    def _fit_feature(self, feature: pd.Series, feature_type: str) -> FeatureInfo:
        ...
        if feature_type == "discrete":
            encoder = get_feature_encoder(self.categorical_encoder, self.cat_encoder_params)
        else:
            encoder = get_feature_encoder(self.continuous_encoder, self.cont_encoder_params)
    ...
    def _transform_feature(self, column_transform_info: FeatureInfo, feature: pd.Series) -> pd.DataFrame:
    ...
    def transform(self, raw_data: pd.DataFrame) -> pd.DataFrame:
        ...
        for column_transform_info in self._column_transform_info_list:
            ...
            column_data_list.append(self._transform_feature(column_transform_info, feature))
    ...
    def inverse_transform(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for column_transform_info in self._column_transform_info_list:
            ...
            recovered_feature = self._inverse_transform_feature(column_transform_info, column_data)
    ...
    def n_features(self) -> int:
        return np.sum(
            column_transform_info.output_dimensions for column_transform_info in self._column_transform_info_list
        )
...
class BinEncoder(TabularEncoder):
    ...
    def _transform_feature(self, column_transform_info: FeatureInfo, feature: pd.Series) -> pd.DataFrame:
        ...
        return pd.DataFrame(out.values[:, 1:].argmax(axis=1), columns=[bgm.feature_name_in])

    def _inverse_transform_feature(self, column_transform_info: FeatureInfo, column_data: pd.DataFrame) -> pd.Series:
        if column_transform_info == "discrete":
            return super()._inverse_transform_feature(column_transform_info, column_data)
        bgm = column_transform_info.transform
...
class TimeSeriesTabularEncoder(TransformerMixin, BaseEstimator):
    ...
    def fit_temporal(
        ...
        temporal_arr = np.swapaxes(temporal_arr, -1, 0).reshape(len(temporal_features), -1)
        temporal_df = pd.DataFrame(temporal_arr.T, columns=temporal_features)

        self.temporal_encoder.fit(temporal_df)

        # Temporal horizons
        self.observation_times_encoder = MinMaxScaler().fit(np.asarray(observation_times).reshape(-1, 1))
    ...
    def fit(
        ...
        # Temporal
        self.fit_temporal(temporal_data, observation_times, discrete_columns=discrete_columns)
    ...
    def transform_observation_times(
        ...
        horizons_encoded = (
            self.observation_times_encoder.transform(np.asarray(observation_times).reshape(-1, 1))
    ...
    def transform(
        ...
        temporal_encoded, horizons_encoded = self.transform_temporal(temporal_data, observation_times)
    ...
    def inverse_transform_observation_times(
        ...
        horizons_decoded = (
            self.observation_times_encoder.inverse_transform(np.asarray(observation_times).reshape(-1, 1))
            .reshape(len(observation_times), -1)
            .tolist()
        )
    ...
    def inverse_transform(
        ...
        temporal_decoded, horizons_decoded = self.inverse_transform_temporal(temporal_encoded, observation_times)
    ...
    def activation_layout(self, discrete_activation: str, continuous_activation: str) -> Tuple:
        ...
        ), self.temporal_encoder.activation_layout(discrete_activation, continuous_activation)
...
class TimeSeriesBinEncoder(TransformerMixin, BaseEstimator):
    ...
    def fit_transform(
        ...
        return self.fit(static, temporal, observation_times).transform(static, temporal, observation_times)
