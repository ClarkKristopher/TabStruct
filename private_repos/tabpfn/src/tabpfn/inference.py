...
@dataclass
class InferenceEngineNoPreprocessing(InferenceEngine):
    X_trains: Sequence[np.ndarray]
    y_trains: Sequence[np.ndarray]
    ensemble_configs: Sequence[EnsembleConfig]
    model: PerFeatureTransformer
    ...
    def prepare(
        ...
        *,
        ...
        force_inference_dtype: torch.dtype | None,
        save_peak_mem: bool | Literal["auto"] | float | int,
    ) -> InferenceEngineNoPreprocessing:
        """Prepare the inference engine.
        ...
        Returns:
            The prepared inference engine.
        """
        # Use shallow copy on purpose to allow backpropagation for the same inputs (i.e., compute gradients for the same inputs)
        X_trains = [X_train] * len(ensemble_configs)
        y_trains = [y_train] * len(ensemble_configs)
        
        return InferenceEngineNoPreprocessing(
            X_trains=X_trains,
            y_trains=y_trains,
            model=model,
            ensemble_configs=ensemble_configs,
    ...
    def iter_outputs(
        ...
        *,
        ...
        if self.force_inference_dtype is not None:
            self.model = self.model.type(self.force_inference_dtype)
        for X_train, y_train in zip(
            self.X_trains,
            self.y_trains
        ):
            X_train = torch.as_tensor(X_train, dtype=torch.float32, device=device)  # noqa: PLW2901

            X_test = torch.as_tensor(X, dtype=torch.float32, device=device)
            ...
            with (
                torch.autocast(device.type, enabled=autocast),
                torch.inference_mode(not requires_grad),
            ):
                output = self.model(
                    *(style, X_full, y_train),
                    only_return_standard_out=only_return_standard_out,
