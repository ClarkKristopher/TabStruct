...
class TabPFNClassifier(ClassifierMixin, BaseEstimator):
    ...
    def __init__(  # noqa: PLR0913
        ...
        *,
        ...
        self.fit_mode: Literal["low_memory", "fit_preprocessors", "fit_with_cache", "fit_no_preprocessors"] = (
            fit_mode
        )
    ...
    def predict_logits_with_grad(self, X: XType) -> torch.Tensor:
        check_is_fitted(self)
        
        assert self.fit_mode == "fit_no_preprocessors", "predict_logits_with_grad only works with fit_no_preprocessors mode"

        outputs: list[torch.Tensor] = []

        for output in self.executor_.iter_outputs(
            X,
            device=self.device_,
            autocast=self.use_autocast_,
            requires_grad=True,
